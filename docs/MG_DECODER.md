...
- **Entry field `0f` spells a MISSING font-file hash as the sentinel `EMPTYHASHFFFFFFF`** —
  consume it (bailing hid field 12 behind it). Presence of field `0f`, including
  this sentinel, marks a computed entry; an omitted pixel flag means AUTO.
...
- 0920 nested chart overlays also inherit `2e` through sparse slash-record
  template slots (`mgStubTrailerField`), just like grow/stretch. Own explicit
  false wins; never follow a bare instance's component reference for this
  positional property. Source slot `3:48511` carries `2e 01`; composed chart
  overlays omit it. UI slimming retains `layoutPositioning`, and the importer
  replays it after the parent has auto-layout. Without this, the plot consumes
  flow height and collapses the grid, moving the horizontal axis above the bars.
...
### Structural instance fallback (0920)

Before UI slimming, `mgMarkStructuralInstanceFallbacks` compares ordered descendant
node-type signatures with the reachable master. Added/removed children or a changed
nested node type set record-level `instanceStructureFallback: true`. The importer
restores these instances as editable frames and does not enqueue a later relink;
`mainComponentId` remains as source provenance. Such roots do not initiate descendant
slimming. Compatible outer instances can still use an already-restored fallback
subtree in their master. This is structural compatibility only, not a guarantee that
same-topology vector overrides match. The 0920 fixture flags 15 records; 0806 is
unchanged. `compare_mg_import.js` deep props do not cover this record-level flag.
...
## 2026-09-19 · 测试集 0920「测试b端」— 全编辑器导出的变量/别名/trailer 补全

Fixture: `测试集/测试集 0920/测试b端-20260919-182243.mg` + 同名 `.zip`（单页 2911 条基准记录，
B 端后台三屏：列表 ×2、详情）。基线比对 deep 15948 → 1429、paint 1355 → 4、index 124 → 0、
child-order 41 → 0、text 3 → 0；Extra 1100 = 96 个外部库 master + 其 1004 条子树（预期）。

### 第五种实例形态：stub 之下的 TYPED bare 子记录（Extra 1779 → 1100，index/child-order 归零）
实例的直接子节点是 slash stub（`3:13781/3:06938`），但 stub 自己的 childIds 是完整的
TYPED bare-id 记录（`3:13786` "1"、`3:13790` "*#color_alpha "），没有 `1a`。基准证明这些
bare id 就是 API 的画布子节点 id。规则：展开 walk 到任一 clone 时，若其 raw childIds 含
TYPED bare 子记录，就**不再克隆模板子节点**（`mgExpandInstances` walk 顶部，与实例级
"fully materialized" 判定同一谓词）。此前每个这样的容器都被塞成 6 个子节点（3 模板克隆 +
3 真记录），633 个实例、1779 条多余记录。

### paint `09` 不是 SOLID 的 opacity（deep −13500，paint 1355 → 83）
1350 条 SOLID paint 与 API zip + MasterGo 自渲染 PNG 交叉表：**只要 `08` 带颜色，其 alpha
就是 opacity，`09` 一律忽略**。变量/样式 paint 大量写 `09 0.88/0.45/0.25/0.06` 而 `08` alpha
= 1（`中性文本色/强调 accent` 3:04316：`08 a=1`、`09 0.88`，渲染与 API 均为 1）；
`colorTextHeading` 3:04313：`08 a=0.44`、`09 0.88`，真值 0.44。旧规则"非 1 的 `09` 覆盖 alpha"
把整页 token 着色的文字/描边压成 88%/45%/25%。`mgPackage.test.js` 的 opacity 用例已改到该语义。
0806 汇总集回归 0 新增行（该集没有 `09 ≠ alpha` 的 paint）。

### paint 尾部新 tag：`0e <b>`、`0f <variable id>\0`（fills.length 0→1 ×138）
全编辑器 paint 在 `0d {…}` 之后可跟 `0e 02`（78 条）与 `0f 7545:209015\0`（别名到库变量，
`color-Primary`）。未知 tag 会让 `mgParsePaintRecord` 丢掉整条 paint——品牌主色的
文字/图标全部 fills=[]。两者顺序消费即可。

### 变量别名表（fills.length 55 → 0）
一个颜色变量可以不带任何 paint 子记录，而在 **id 关联表**里逐 mode 指向另一个变量：
`01 <A>/<mode> 00 02 <B> 00 04 01 05 <A>/<mode>:0:0 00 06 02 00`
（`错误色/color-error` 3:06851 → `.red-6` 3:06852，三个 mode 各一行；集合 3:04249 的
mode 为 `M:2 慧充` / `7545:209329 DiDi Volt` / `7545:209625 其他平台`）。`mgScanPaints` 扫描
该表，paint 为空的变量沿别名链（≤8 跳）借用目标变量的 paint。变量条目本身的
`08 <7 字节 varint>` 高位在所有条目上相同（`80 90 af 07`），是版本戳不是颜色。

### trailer 新字段（deep −600）
- **`1f 01` = layoutAlign STRETCH**。全部 raw 记录交叉表：273/273 STRETCH 带 `1f 01`，
  0/976 INHERIT 带。关闭 `MG_DECODER_UNKNOWN_FIELDS.md` 里"STRETCH 来源字节未定位"一项。
- **`30 <n> { 01 <组件属性引用> 00 02 <kind> } × n`**：属性绑定表（`Icon#513,53` ↔
  `5939:437272`）。1735 条 trailer 带它；旧的通用 2 字节跳过落进字符串里，整条 trailer
  返回 null——该记录的 tag 26 缩放、`1f`、`20` 全丢（图标实例 3:27927 的 `26 0.667` 就是这样
  丢的）。
- `28 <id>\0`、`29 <JSON>\0`、`33 <id>\0` 字符串字段；`32 <zero-float>`。
- **`20`（layoutGrow）对所有节点类型生效**：0920 有 238 个 FRAME、20 个 RECTANGLE、240 个
  实例子 TEXT 带它；此前赋值写在容器 meta 分支里，TEXT 永远拿不到。
- **stub 自己的 trailer 不写 `20`/`1f` 时继承模板子节点的**（216 条 raw slash override
  交叉表：stub 从不自己写这两个 tag；209/216 grow 与模板一致，其余在非 auto-layout 父级下）。
  `mgStubTrailerField` 沿 `templateNode`/`templateRef` 走，但**只在 id 仍是 slash stub 时
  继续**——INSTANCE 记录自己的 `1a` 指向组件，组件 trailer 描述的是组件在其父级里的槽位
  （16 个 DataEntry 容器因此曾被误标 STRETCH）。

### 嵌套实例 tag 26 与合成子节点（strokeWeight 210 → 26）
模板里的嵌套 INSTANCE 记录带 `26 <scale>` 时（3:27927 `26 0.667`，24→16 的图标），合成克隆
的 `effScale = 父 effScale × 模板 26`。基准 strokeWeight 0.667 = 1 × 0.667，与按约束缩放得到
的盒子比例（14/24 = 0.583）无关。0806 汇总集 0 新增行——0806 规则针对的是**非实例**记录上
的环境缩放，两条规则不冲突。

### override 记录可以挂在模板路径的任意深度
`3:30658/3:26786/3:11624`（"商户"）与更浅的 `3:26786/3:11624`（"已关闭"）并存；
`3:22167/3:10542` 给 walk 路径为 `3:28657/3:22167` 的矢量换色。查找顺序：完整 tplPath →
每个更短的后缀 → flat rootKey；嵌套实例 job 的 `tplSide` 保留 override 记录自己的路径
（否则子 job 只会找到浅层记录）。text 3 → 0、paint 14 → 4。

### 渐变的空 stop
`05 02 | 00 | 01 <1.0> 02 <a=0 …> 00`：紧跟计数的 `00` 是"全默认"stop——position 0、颜色 =
默认填充灰 #D8D8D8、alpha 1（卡片 hover 渐变 3:11339）。丢掉它只剩一个 stop，整条渐变退化成
默认灰实心。

### 实例子节点的 vectorNetwork 基准尺寸
`mgSynthesizeInstanceChild` 的 `tplW/tplH` 取模板 SLOT 的尺寸，不能取 override 记录的——
`3:26131/3:10899` 自己写的是缩放后的 12.5，模板 Vector 是 18.75；按 override 读比例恒为 1，
154 个嵌套图标的顶点没缩放（画大 1.5 倍被裁）。

### importer（zip / mg 共用）
实例子层的 `layoutGrow` / `layoutAlign` 现在进入 `pendingInstanceLayoutOverrides`，在父级
拿到 layoutMode 后回放（`flushInstanceChildLayoutOverrides`）；UI 精简键表 `MG_SLIM_LAYOUT_KEYS`
同步加了这两项。0920：Form Item 实例里的 DatePicker 实例存 grow 1，组件自身 320 宽，
不回放就撑出 264 的表单项。

### 保留残差（1429 deep）
`regions[].loops` 顺序（380，渲染等价）、实例 FILL 宽度由 Figma 自解（129 + x/transform）、
VECTOR 顶点 cornerRadius 与节点级 -1 哨兵（194 + 153）、SVG fallback 四字段（128）、
strokeWeight 26 条（其中仅 2 条有可见描边）、实例名跟随组件集名（23）、图像滤镜字段映射（3）、
`进行中` 一处 mode 颜色（1）。

### 0920 follow-up: image adjustments, computed line height, minimum width

- Paint `0d` slots are sequential: `01 contrast`, `02 exposure`, `03 highlights`,
  `04 saturation`, `05 shadows`, `06 temperature`, `07 tint`, `08 hue`.
  The three header image paints match ZIP values exactly, including highlights
  `0.3305203021`, shadows `-0.7716525793`, temperature `-0.0500000007`, and
  tint `-0.1071912944`; these four controls were previously consumed but dropped.
- Font entry `0f EMPTYHASHFFFFFFF` is still a computed entry. Without `06` pixel
  or `07` percent flags, its stored height is not an explicit pixel line height.
  Entry `3:48143` stores height 10 while the ZIP specifies AUTO and a 17px box.
  This rule corrected seven matched text records in 0920 without a line-height
  regression in that fixture. Explicit pixel/percent flags remain authoritative.
- Trailer `32 <zero-compressed float>` is `minWidth`. Component `3:22162`
  contains `32 85 00 00 28` (74); its button instances keep 74px despite text plus
  padding totaling 60px. Inherit the template value when absent, scale with the
  instance, and retain it through slim instance transport. The historical ZIP
  omitted size limits; its 215 missing `layout.minWidth` properties are expected
  structural differences. New exports include min/max width/height, and the
  importer applies these limits during deferred layout and instance overrides.

### 0920 variable mode resolution

Alias rows are not ordered by the collection's modes. `3:28016` (color-info)
first appears as `3:28016/7545:209625 -> 3:28000` (blue), but the collection
`3:04249` declares mode `M:2` at sort code `a0` (慧充), before `a1`/`a2`.
Its `3:28016/M:2 -> 3:05435` alias resolves to the expected cyan. Read the
mode-definition grammar `01 <mode> 00 02 <sort> 00 03 <name> 00 04 01 00`
and choose the earliest declared mode when resolving default aliases. Keep
first-row behavior only when mode definitions are unavailable. Per-node explicit
mode overrides are not decoded by this default-mode rule.
