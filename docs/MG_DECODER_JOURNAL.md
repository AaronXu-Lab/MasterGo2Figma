...
## 2026-08-06（汇总集，第三轮）— 拿 MasterGo 自己导出的 PNG 当标准答案

前两轮都是拿基准 zip 当标准答案。这轮用户换了个更狠的：**直接在 MasterGo 里把
`首页普通版` 导出成图片**，放进 `0806 图片` Page，让我用眼睛对。这是一次质变——
zip 是"MasterGo API 说的值"，PNG 是"MasterGo 屏幕上画出来的样子"，后者能抓到
**两个包一致、但导进 Figma 就是不对**的那一类问题。

方法：两张 PNG 都按 2× 导出（808×1750），逐行统计差异像素比例，得到"热区"行段，
再对热区做并排放大裁切来看。行剖面把 875 高的画板压成 16 个区间，5 分钟定位完毕，
比来回滚动截图快一个数量级。

### 1. 快/慢 标签：圆形 + 双倍字号 → 嵌套实例从来没被 rescale 过

芯片渲染成正圆（该是圆角矩形），里面的"快"字大到被裁掉一半。包里查：
`fontSize: 11.85`（= 22 × 0.5387）、`width: 11.85` —— **数据全对**。
再查 Figma 里的真实节点：`22 × 32`，正是组件母版的原值。

`applyInstanceRecordState` 只在 `layerRecord.instanceScale` 存在时调
`InstanceNode.rescale()`，而发射端写的是 `n.trailer.scaleFactor`。给发射点挂日志，
30 条 INSTANCE 记录一目了然：**7 条嵌套实例 `effScale` 都是 0.5387，`trailerScale`
全是 undefined**——合成出来的克隆本来就没有自己的 trailer。于是 Figma 只把外壳
resize 了，锁定的组件子节点保持组件空间的 22px 字号和 7px 圆角，塞进 17px 的盒子里，
圆角一 clamp 就成了正圆。

改成 `record.instanceScale = n.effScale`。（`effScale` 对顶层实例本来就等于自己的
trailer factor，两种情形一条式子覆盖。）

**教训**：**"包里的值对"不等于"导进去对"**。实例是唯一一处 Figma 不接受子节点几何、
只接受一个 `rescale()` 的地方——这类"必须由父级重放"的属性，验证必须落到 Figma 节点上，
比较器看不见。

### 2. 地图定位大头针是黑的 → 深层 override 的 key 是**扁平**的

`圆形 96` 包里写 `#000000 @0.9`，MasterGo 画的是蓝色。原始记录里确实有一条
`24:1747/24:0946`，`paintRef 24:0768`（蓝）——但展开时没被用上。

看 `mgExpandTemplateInstances` 的 `tplPath` 链：它逐层拼 `cur.tplPath + "/" + lastSegId`，
**某一层没有 override 记录就退回原始模板 id**，链子从那里断掉。而 MasterGo 的 override
key 是**扁平挂在实例 id 上**的：`24:1747/24:0946` 的 `02 <parent>` 甚至指向一条根本不
存在的 `24:1747/24:0944`。中间的 `组 1136` 没有自己的 override，链子一断，底下所有
override 全部失联。

加一个 `rootPath`（= 本 job 的实例段，全程不变），逐层 key 未命中时回退到
`rootPath + "/" + lastSegId`。

**教训**：**id 的层级长相 ≠ 它的命名空间**。`a/b/c` 看着像路径，实际是
`<实例>/<模板节点>` 的二段式；照着"路径"去逐层拼，遇到稀疏层就断。破解 id 方案时，
先找**没有中间节点**的那条记录（这里是 parent 指向不存在记录的 `24:1747/24:0946`），
它会直接告诉你命名空间的根在哪。

### 3. 消息通知那行文字冲出卡片、`10` 角标不见了 → `03 03` = TRUNCATE

Figma 里那条文字被 hug 成全长，把同一 auto-layout 行里的 `10` 角标顶到了 x 564
（行宽只有 378）。MasterGo 存的是**未截断的全串**，靠渲染时截断，所以 autoResize
解错就是这个后果。

`1c 08` 的 `03` 字段直方图：`0`×218、`1`×18、**`3`×1**——而那唯一一条就是这行文字。
基准 zip 对它写的是 `textAutoResize: "TRUNCATE"`。所以 `3` = TRUNCATE。

但光解对没用：Figma 已经**废弃并拒绝** `TRUNCATE` 这个值，赋值抛异常被 `trySetText`
吞掉，节点就停在默认的 hug 上——**zip 导入路径一直有同一个 bug**。applier 改成
`TRUNCATE → textAutoResize NONE + textTruncation ENDING`。

**教训**：直方图里的**单例**最值钱。218/18/1 这种分布，那个 1 就是"标准答案交叉表"
里唯一一行——不用猜，直接去看它在基准里被写成什么。

### 4. 附近充电按钮的纸飞机图标消失 → 自己的尺寸被自己的 tag 26 乘了一遍

图标容器 24×24 变成 1.55×1.55（= 24 × **0.12** × 0.5387），clip 掉了里面 11.2×11.66
的矢量。0.12 是这个节点自己 trailer 里的 tag 26。

肇事代码是 2026-07（tag 26 还被当成"实例缩放"时）写的：raw 记录的显式尺寸乘自己的
scaleFactor，"除非它已经等于 模板 × scale"。但那个保险丝的 `nodes[n.templateRef]`
在继承阶段还是 null，**从来没熔断过**。挂日志一看：命中 32 条，**32 条全部**在基准里
是乘之前的值。整条规则删掉。

**教训**：**带"除非"的启发式规则要验证"除非"分支真的会走到**。这条规则活了一个月，
保险丝一次都没响过，而它保护的正是唯一正确的分支。挂个计数器就能发现——成本几乎为零。

### 5. 地图纹理跑到标签栏底下 → ABSOLUTE 的 Group 丢了坐标

`蒙版组 3` 两个包都写 `layoutPositioning: ABSOLUTE` + `x: 170.76`，Figma 里却在
`(12.93, 12.93)`——父容器的 padding 角。这是纯 importer 问题（**zip 路径同样中招**）。

`createFinalizedContainerProps` 对原生 Group 只保留 `layoutAlign/layoutGrow/
layoutPositioning`，坐标交给 `figma.group()` 自己算。平时没问题，唯独 ABSOLUTE 不行：
延迟布局第一遍先给父级写 `layoutMode`，Figma 当场把这个 group 吸进流式槽位；第二遍才
把它翻回 ABSOLUTE——此时手里已经没有坐标可以还原了。ABSOLUTE 时一并保留 x/y。

**教训**：**"由 Figma 自己算"和"延迟应用"是一对危险组合**。凡是"先让 Figma 算，回头
再改父级布局"的路径，都要问一句：改父级的那一刻，Figma 会不会把算出来的东西冲掉。

## 2026-08-26 临时测试 pass：一条被点号毁掉的正则，和两个"其实没丢，只是看不见"的导入 bug

新测试集 `测试集/临时测试`（788 条基准 + 124 条外部库 master，编辑器导出 + share 式斜杠
override）。起点：deep-prop 1304 / font 166 / geometry 15 / transform 6 / index 2。
终点：deep 60（全部为已接受残差：缺失字体的 fontWeight 0、导出器专属 SVG 通道、单行文本的
paragraphSpacing）、font 4、其余全零。

### 1. 一个点号毁掉 22/27 个字体样式条目（deep 1304 里的 ~900 行）

`mgScanFontStyles` 的 family 校验正则 `[A-Za-z0-9 +-]` 不含 `.`，而这个文件的主力字体叫
"Alibaba PuHuiTi 3.0"。条目被丢弃后 fontSize 回退 16px 默认值，塞不进 11px 的定高文本框，
整页文字溢出/截断——mg 导入"明显比 zip 差"的主观印象几乎全部来自这一个字符。
同一批条目还带来两个新拼写：`12` 字段变成 JSON（`{"fontStyle":"65 Medium","opsz":"auto"}`），
`0f` 的空哈希写成哨兵串 `EMPTYHASHFFFFFFF`（旧代码在这里 break，把后面的 12 也一起丢了）。

**JSON fontStyle 不能无脑信**。0806 回归立刻抓出反例：PingFang 条目的 JSON 是中文别名
"中黑体"，而基准（插件 API）报英文 "Medium"。判别式：JSON 名与 psName 样式去空格后相同 →
用 JSON 的写法（拿到 "65 Medium" 的权威空格，胜过 camel-case 猜测）；不同（本地化别名）→
psName 的英文赢。CKTKingKong 那 2 行（API 真报"粗体"）回到旧的已接受残差。
**教训：显示名字段有两种身份——"权威拼写"和"本地化别名"——必须拿第二个来源交叉。**

### 2. 矩形逐角圆角：`01 04` 是数组，不是单浮点

`1c 03 01 04` 后面是 4 个零压缩浮点 [TL,TR,BL,BR]。旧代码读单个 raw float——在全部四角相同
的旧 fixture 上恰好等价（读到第一个元素），直到 [0,0,4,4] 出现：首字节是零压缩的 `00`，
读数错位落进载荷，铸出 2.31e-41 的 denormal 垃圾。**教训：读对了结果 ≠ 读对了语法；
"恰好等价"的读法会在第一个非对称样本上爆炸。**顺带破解 trailer `24` = cornerSmoothing
（但只对 RECTANGLE 生效，VECTOR 上 API 报 0——交叉表 6/6 vs 33/33），以及 VECTOR 的
cornerRadius 其实是顶点半径的镜像（同值=标量，异值=-1）。

### 3. 头像缩水到 0.78 倍：override 记录的坐标系

嵌套实例链 组16713 → 头像@50（组件 3:0816 是 64）→ _Base/Avatar。斜杠 override
`3:0877/3:0817` 存的是 50——在"头像@50"上下文里的**终值**。展开时拿它当模板值、按组件父
（64）→ 克隆父（50）再映射一次：50 × 50/64 = 39.06。修法：overrideKey/rootKey 命中的 t
用 `nodes[t.parent]` 做约束缩放的旧父（它的值就活在那个坐标系里）；bare mirror（全量导出）
维持模板语义。**教训：每个记录的数值都有"所在坐标系"；share override 的坐标系是它的实例
上下文，不是组件。**

### 4. 信号/WiFi 图标"丢失"——其实一根矢量都没丢

视觉对照抓到状态栏只剩电池。层层排查 restore 路径无果后，用 use_figma 手动复现导入序列——
**完全正常**。再用脚本直接 dump 导入结果的真实子树（get_metadata 会省略深层子节点，误导了
一小时）：GROUP → BOOLEAN(EXCLUDE) → 4 VECTOR 全在，每个矢量 fills=1，唯独 **boolean 自己
fills=0**。MasterGo 把复合体的填充放在**外层** boolean 上（内层无填充）；
`promoteSingleBooleanChild` 提升唯一子节点时只改名、不搬填充——Figma 的 boolean 用自己的
fills 渲染，于是整个图标透明。zip 和 mg 两条路径同样中招。
**教训一：'看不见'≠'没还原'，先 dump 真实节点树再定性。**
**教训二：get_metadata 的 XML 会静默截断深层子树，取证要用 use_figma 遍历。**

### 5. 标签行背后的灰条：默认填充不该有孪生

标签行底下多了一条 #D8D8D8。它是 `paintFilledMaskTwins`（0806 引入：MasterGo 会渲染蒙版
自己的填充）插的孪生——但这个蒙版的填充是 216/255 的**默认占位灰**（MasterGo 蒙版新建即有、
从不渲染；.mg 里它甚至是无颜色的空 paint 记录，D8D8D8 是解码器合成的）。基准里 4 个蒙版
交叉表：橙色渐变蒙版（附近商家活动卡）MasterGo 真的画了 → 孪生正确；D8D8D8 蒙版没画 →
跳过。**教训：给渲染差异打补丁时，'有填充'要区分'用户画的'和'工具默认的'。**

### 6. 导出设置注册表（顺手破解）

`01 <entryId> 00 02 <节点id> 00 03 <sort> 00 06 <scale 浮点> [07 <格式: 2=SVG>]
[08 <后缀> 00] [09 01] 00`——parent 指向拥有它的节点。首页/正 挂 `06 <2.0> 08 "@2x"`，
每个可导出切图挂 `06 <1.0>`。0806 的 `0:100`（PNG+SVG 双设置）证明了 `07 02` = SVG，
也证明导出顺序是按格式（PNG 先）而非排序码。实例克隆不继承（基准克隆切图为 []）。

### 7.（第二轮回归）画布组件被当成库 master 删除 —— key ≠ off-canvas

用户重导入后：首页的 组 16567 整块空白、关注-公示 少了第一行卡片。两者同根因：本轮新加的
`07 01 <描述> 00 03 <库key>` 解析给它们种上了 `libraryKey` → `libraryMaster` → importer
收尾把它们从画布上删了。但它们是**画布上的真组件**（从库同步来的组件也带 key，且就住在
frame 里）；真正的 off-canvas master 全部是**页面根**（parent = 页面 owner token）。
修法：`record.libraryMaster` 加父级门控（`!nodes[n.parent]`）。
**教训一：比较器对"importer 删了什么"全盲——record 都在、digest 全零，画布上却少了两块。
解码器改动涉及删除类标记（libraryMaster / residue skip）时，必须重导入或数画布顶层块数。**
**教训二：同一个字段（库 key）在不同宿主位置语义不同；用"位置 + 字段"联合判别，别用字段单判。**

### 8.（第三轮）"字体装了却不还原"——GB18030 字符集版样式名 L3

用户装了 Alibaba PuHuiTi 3.0，但 zip 和 mg 导入里正文全部回退 Inter。画布取证：
65 Medium / 85 Bold / 75 SemiBold 都成功还原（族名匹配没问题），挂掉的只有
`55 Regular L3` 这一档（68 个节点）。根因在 importer 而非解码器：MasterGo 记录的是
字符集扩展版样式名（L3 = GB18030 实现级别 3），用户装的是普通版（样式名 `55 Regular`），
`fontLoader.ts` 的样式匹配是归一化后精确相等 → 整档判失配。
修法：`normalizeFontStyleForMatch` 归一化时剥掉尾部 `l\d+` 字符集标记（剥空则保留原值，
避免样式名就叫 "L3" 的极端情况）。record 保持忠实原值不改——装了 L3 版的用户仍先走精确命中。
修复后无需重导入：插件 UI 的"刷新字体"入口（`refreshMissingFontsInDocument`）会对带
`[Font Missing]` 前缀的节点重新解析并就地恢复。
**教训：zip 和 mg 同错且错的是"部分字重"时，先按字重分组看哪些成功——成功的字重排除族名
问题，把嫌疑压缩到样式名匹配一条线上。**

### 9.（第四轮）橙色 banner 卡过饱和——蒙版补漆孪生错画了"仅形状"蒙版，破解 trailer `1e`

用户指出首页"附近商家活动"橙卡与 MasterGo 参考图差异大（zip、mg 同错）。像素取证：
卡片中心基准 (255,224,206) ≈ 渐变直接压白底；导入 (255,178,131) = 纯 #FFB283 底——
paintFilledMaskTwins 给蒙版 `矩形 148535` 补的孪生画出了 MasterGo 从不渲染的底色。
关键反例是蓝卡：其可见背景来自**旁边一块非蒙版的同渐变兄弟矩形**（MasterGo 惯用
"蒙版 + 同款画纸兄弟层"结构），孪生只是撞色无害重复——**"补漆后视觉正确"不能证明
"蒙版被渲染"**。但 0806 的 tab-bar `圆形 865`（无兄弟画纸层）当年实证需要补漆，
说明 MasterGo 有的蒙版画、有的不画 → 必有格式位。
hexdump 对比两条记录 → 0806 的圆多出 trailer `1e 01`；全量普查：0806 蒙版 14/17 带、
临时测试 4/4 不带，与全部已知渲染事实零冲突。
修法：解码器把 isMask 记录的 `1e` 发成 record 级 `maskRendersFill`（比较器不可见，
diff 数字全不变），importer 仅在其不为 false 时补孪生；zip 导入无此字段维持原行为
（所以 zip 页橙卡仍旧过饱和——这是导出链拿不到的位，属 mg 反超 zip 的案例）。
画布预演（删孪生后重截图）：橙卡 7 个采样点与基准图最大通道差 1/255。
**教训一：验证补漆类修复时先排查"兄弟画纸层"——撞色重复会把错误规则伪装成正确。**
**教训二：两个 fixture 各持一半真相（0806"要补"、临时测试"不能补"）时，答案几乎
一定是还没破解的格式位，去 hexdump 差字节，别选边站。**

## 2026-09-06 · 测试 0906 外部组件恢复

- 现象：nav row 被压成 Frame，多个导航图标缺失，steering wheel 的 Boolean 高度拉长。
- 证据：原始记录中已有完整外部母版（nav row `3:0182`、方向盘 `3:0040`），但空 code 导致它们未进入页面可达集合，实例没有输出 mainComponentId。
- 修复：按每页实例引用补齐母版依赖闭包，保留变体集合；复用 importer 的临时母版创建、实例化和清理，不改 Boolean 几何启发式。共享依赖按页重编号，避免全局重复 ID。
- 第二个独立原因：带 instanceRef 的真实 override 记录，scalar 07 缺失表示可见，不能继承隐藏槽位。224 条与 zip 交叉表无反例（201 可见、23 显式隐藏），见规格文档。
- 0906：缺失 0，新增 2148 条全部是 62 棵依赖母版树；deep props 311 → 302，geometry 61 → 59，transform 52 → 50。record 级 mainComponentId 不在比较器范围，必须真实重导入验证。
- 回归：本地仅有 0906 和「插件测试 汇总.mg」。后者以 detached HEAD 解码生成 zip 做 A/B：原有 1369 条全部属性不变，新增 461 条母版依赖；不是历史原始 zip 基准复测。
- 两端构建通过，30 项测试 28 通过、2 项失败；同两项失败在 HEAD 26 项测试中已存在。新增 4 项覆盖闭包、变体、可见性和跨页引用隔离。
- 最终构建已通过开发插件真实重导入（开启 UI 的实例子层瘦身）：6 个 nav row 均为 INSTANCE，5 个缺失图标恢复可见；Dark 的 steering wheel/Union 为 36.9873 × 36.9873（旧 mg 为 36.9873 × 48.7559），页面剩余 COMPONENT/COMPONENT_SET 为 0。交付页「临时测试用_mg_修复0906」（44:7422），中间试验页已删除。
- 已做 image / zip / mg 三联截图对照。方向盘蓝色实体和背景采样最大通道差 0；参考 PNG 的边缘像素化与 Figma 抗锯齿仍有差异，字体和其余历史残差不计为本轮全像素复原。

## 2026-09-06 · 杂记副本：图片失败与空 fills 分开诊断

- 输入 `可同步测试集/杂记 副本.mg`，约 294 MB，6 页、1161 条记录；59 张图片（51 PNG、7 JPEG、1 HEIC）。
- 原 Figma 国庆来回记录中的 `VID20221002080958` 为灰色缺图占位。对应资源是 HEIC；浏览器 UI 使用本地 bundled heic-to/csp 转 PNG 后再流式上传，沿用全部原始资源别名。普通图片无需转换。
- “图片失败后其他 fills 丢失”实际包含第二个根因：解码阶段拒绝 SOLID paint 中的完整 inactive image/filter 对象。MasterGo UI 的 `0 西湖` 为 #3D3D3D，解码修复后与之相符。769 个空 fills 恢复（680 TEXT、86 FRAME、3 RECTANGLE），原来已有 fills 和 strokes 均未改变。
- 防连带失败：HEIC 转换错误回落原资源并走缺图记录流程；资源拼接校验纳入 catch/finally，单图失败继续后续资源并释放块；fills/strokes 原子赋值失败时按 paint 分别恢复，保留其他 paint 的 transform 等属性。
- 真实完整导入成功：6 页1161层，无图片缺失通知；HEIC 图层成为有效3072×3072图片，国庆页灰色占位数0；杭州页空 fills 文本数0。新增六页均标“（修复验证）”，原六页保留。字体仍有138个文本缺失，未混同为图片/填充问题。
- 回归：35测试33通过，2既有失败同前；两端构建通过。旧「插件测试 汇总.mg」1830条与本轮前记录逐条完全一致。0906输入已不在当前工作区，未重复宣称已做该文件新一轮实测。

## 2026-09-06 · 杂记更新：跨页母版晚于实例

- Figma `5:7364`「交通卡 3」为 Frame；源记录 `542:0775` 已有正确 mainComponentId `378:1687`。同页另外两个交通卡实例也指向该母版。
- 实例在第4页「横店旅行安排」，母版在第6页「广东->浙江 通勤方案」。解码器数据正确；原 importer 在第4页收尾重试后直接清空队列，未来母版到达时无任务可重试。
- 修复 importer `retryDeferredInstanceRelinks`：母版尚未就绪时保留队列，只保留该实例及子孙覆盖记录；每页收尾继续尝试，以原父级和索引替换 Frame。已删除的 shell 自动丢弃，不增加还原计数。
- 解码器未改动。新增实际重链函数的跨页回归，验证中间无母版页面不会清空任务、子层覆盖保留、无关页面记录不被持有、替换保留兄弟顺序。36测试34通过，2既有失败；两端构建通过。
- 实际插件仅选实例页和后续母版页重导入（2页781层），三个交通卡均出现 Reset instance / Detach instance / Main component 菜单；问题节点的新实例为 Figma `5:12536`。新增页标记“（跨页实例修复）”，原页保留。现有字体缺失不属于本次关系修复。

## 2026-09-06 · 春节规划外部文字样式本地化

- 用户指出外部库字体回退。对照 MasterGo/Figma：源稿标题链接“大标题”，但旧解码53个文本均回退Inter，字号靠高度估计（标题14px）。
- .mg已含完整样式。根因是命名记录04空字段未匹配；另有匿名覆盖03空family、混合run的04标记未消费。修复按证据限定语法；未扩张到所有无前缀旧样式。
- 创建本地TextStyle前复用字体匹配器，使PingFangSC匹配可用PingFang SC；整段/范围分别绑定，UI多文件前缀涵盖textStyleRanges。命名样式外的本地字重覆盖保留。
- 真实导入春节页1页108层成功，无缺失字体提示；Figma标题5:17292绑定“大标题 · 64/77”，混合正文仍显示mixed typography。53文本Inter回退降至0；23整段样式引用、21文本的命名样式范围，其余9个是匿名覆盖。新增页“2023春节回家规划（文字样式修复）”，原页保留。
- 旧汇总1830条记录与本轮前逐条完全一致；42测试40通过、2既有失败；双端构建通过。最终语法收窄后春节页记录与实际验证构建一致。
