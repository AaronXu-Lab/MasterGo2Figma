...
| `0x07` 子标签 | — | **已破解（2026-08-05）** | `07 03 <libraryFileId+nodeId>` = 外部库 master，`07 04 <varint>` = 本地 master（0806 集 22/22 零违例）；外部库 master 打 `libraryMaster` 标记，还原后由导入端删除 |
| `0x1a` | — | 已消费未破义（2026-08-05） | auto-layout 框架上的单字节旗标；**不消费会让对象终止符不可达**，锚定 trailer 整段丢失（layoutGrow / 21 / 22 全没） |
...
| `0x04` | — | 未破解 | 头部字段，义未明 |
| `0x05` | — | 未破解 | 头部字段，义未明 |
| `0x03 <b>` | ×237 | **已破解（2026-08-06 补齐第三个值）** | textAutoResize：`00`=WIDTH_AND_HEIGHT（×218）、`01`=HEIGHT（×18）、**`03`=TRUNCATE**（×1，汇总集的消息通知行；基准 zip 同样写 TRUNCATE）、缺失=NONE。`02` 未观察到 |
...
| tag | 出现频次 | 状态 | 备注 |
|---|---|---|---|
| `0x20` | — | **已破解（2026-08-05）** | 零压缩浮点 = **layoutGrow**（fill container），0806 集 7/7 有 ⇒ 1、439/439 无 ⇒ 0。注意 `20`/`21`/`22` 都是零压缩浮点而非单字节，按 `p += 2` 步进会落在浮点中间截断整个 trailer walk |
...
| 位置 | 状态 | 备注 |
|---|---|---|
| paint 记录字段 `07` / `0c` | 未破解 | 尾部 flag，解析时跳过，无对应基准差异 |
| paint 记录字段 `0d` | **已破解（2026-08-06，汇总集）** | **不是 flag，是"图像调整"子对象**，字段号为位掩码：`01` 对比度 `02` 曝光 `04` 饱和度 `08` 色相；Figma 剩下的 temperature/tint/highlights/shadows 应是更高位，本集未出现。没调整时写成 `0d 00`（空对象），恰好与旧的单字节 flag 读法同宽，所以错了一年没暴露 |
| paint 记录 `0d` 的高位字段（16/32/64/128） | 未观察到 | 需要一张调过色温/色调/高光/阴影的图片样本才能定位 |
